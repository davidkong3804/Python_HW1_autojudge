...
1. 打開批改網頁。兩個方式：
   * **GitHub Pages 網址** —— 第一次要下載 Python 執行環境（傳輸約 5.5 MB、解壓後 11.7 MB），
     網路慢的話 30～60 秒，右上角會顯示百分比；下載完存在瀏覽器快取，之後重開只要幾秒。
   * **本機開（要批很多份時建議這個）** —— 雙擊 `start_local.command`（macOS），
     或在這個資料夾執行 `python3 -m http.server 8765` 再開 `http://localhost:8765/`。
     Python 環境直接從硬碟讀，**約三秒而且完全不用網路**。
2. 等右上角顯示「Python 就緒」。
...
| 放入檔案後（自動開始準備） | 瀏覽器多吃約 **130 MB** 記憶體 |
| 批完一位同學六題 60 筆測資 | 再多約 10 MB，關掉分頁就全部釋放 |
| 硬碟 | GitHub Pages 版在瀏覽器快取放約 5.5 MB；本機版 0 |

Pyodide 就是完整的 CPython 編成 WebAssembly，這 130 MB 是直譯器本身的大小，
換成更輕的 Python 實作（例如 Brython）雖然會秒開，但浮點格式化等細節跟真 Python 不一樣，
**批改結果可能失真**，所以這裡選擇維持 CPython。
...
HW1 六題的測資**每一題都只有一行**，所以正解就是一次 `input()`。
把一行拆成好幾次 `input()` 的同學會因為讀不到第二行而 `EOFError`，**判錯**。

> 「開始批改」那排有一個預設**關閉**的非標準選項
> 「（非標準）相容『一行拆成多次 input()』」，打開後會把那一行拆成逐行輸入重跑一次；
> 命令列版是 `--lenient-input`。除非有特別理由，否則不要打開。
...
| 印完答案後多一個 `input("按 Enter 結束")` | 輸出正確就給分（可用 `--strict-output` 改成算錯） |
